...
public class EventManager : MonoBehaviour
{
    ...
    public bool AddListener(IEventListener listener, string eventName)
    {
        ...
        // PGLog.Log("Adding listener: "+listener);
    ...
    //Trigger the event after the specified time, this should only be used in specific circumstances,
    //the QueueEvent function is usually fast enough for the vast majority of uses.
    public void TriggerEventAfter(IEvent evt, float delay)
    {
    	StartCoroutine(TriggerEventAfterDelay(evt, delay));
    }
    
    public IEnumerator TriggerEventAfterDelay(IEvent evt, float delay) {
    ...
    {
/*
     	foreach (string eventName in m_listenerTable.Keys) {
	        foreach (IEventListener listener in (ArrayList)m_listenerTable[eventName]) {
	        	PGLog.Log("EventManager OnApplicationQuit: listerner remaining: "+listener.GetType().ToString()+" event["+eventName+"]");
	        }
        }
*/
